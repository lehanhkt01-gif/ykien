...
export async function GET(req: NextRequest) {
  try {
    ...
    if (isTemplate) {
      // 1. FILE MẪU CHUẨN ĐỒNG BỘ 100% CƠ SỞ DỮ LIỆU (17 TRƯỜNG)
      fileName = "Mau_Nhap_Y_Kien_Cu_Tri_EaSup_Chuan_CSDL.xlsx";
      rowsData = [
        {
          "STT": 1,
      ...
      // 2. XUẤT DỮ LIỆU TỪ CƠ SỞ DỮ LIỆU
      ...
      const suffix = exportType === "filtered" ? "BoLoc" : "ToanBoCSDL";
      fileName = `Danh_sach_y_kien_cu_tri_Xa_Ea_Sup_${fileDate}_${suffix}.xlsx`;
    ...
    console.error("Lỗi xuất file Excel tại API:", error);
    return NextResponse.json(
